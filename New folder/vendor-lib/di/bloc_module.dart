...
void registerBlocs(GetIt sl) {
  ...
  // User feature BLoCs (vendor also uses user profile/main/etc.)
  ...
  // Vendor BLoCs
  sl.registerFactory<VendorRestaurantBloc>(
    () => VendorRestaurantBloc(
      restaurantRepo: sl<VendorRestaurantRepo>(),
      socketService: sl<SocketService>(),
    ),
  );
  sl.registerFactory<VendorOrderBloc>(
    () => VendorOrderBloc(orderRepo: sl<VendorOrderRepo>()),
  );
  sl.registerFactory<VendorDashboardBloc>(
    () => VendorDashboardBloc(
      analyticsRepo: sl<VendorAnalyticsRepo>(),
      orderRepo: sl<VendorOrderRepo>(),
    ),
  );
  sl.registerFactory<VendorAnalyticsBloc>(
    () => VendorAnalyticsBloc(analyticsRepo: sl<VendorAnalyticsRepo>()),
  );
}
